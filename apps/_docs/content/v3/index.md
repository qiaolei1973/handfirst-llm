...
![神经元、隐藏层、Linear、ReLU 之间的结构关系](/v3/architecture.png)
<!-- gen_v3_arch.py — 三层：一个神经元=Linear+ReLU / 隐藏层=N个神经元 / 完整1→N→1网络 -->
...
![前向传播与反向传播：链式法则把误差从输出端传回输入端](/v3/backprop.png)
<!-- gen_v3_backprop.py — 左侧前向(x→Linear→ReLU→输出→Loss)，右侧反向(Loss→输出层梯度→ReLU'→隐藏层梯度)→收算传 -->
