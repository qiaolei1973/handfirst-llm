...
> 你将学到：多维特征、权重矩阵、矩阵乘法、点积、3D 损失曲面。

v1 到 v4 的模型都在处理**一个特征**——输入是单个 x，输出是 ŷ。但真实世界的数据从来不止一个特征：房价取决于面积、楼层、朝向、周边学校……光知道面积不够。

这一章要把输入从一个数扩展到一个向量。这个变化很小——但概念上，它让你看到了机器学习中最重要的数据结构：**矩阵**。
...
```
        列1=面积  列2=卧室数  列3=楼层
行1     [ 85        2          5     ]    ← 样本 1 的所有特征
行2     [ 120       3          12    ]    ← 样本 2 的所有特征
行3     [ 65        1          3     ]    ← 样本 3 的所有特征
```
...
隐藏层权重矩阵 W_h [N × d]:
          ...
          x₁(面积)   x₂(卧室数)  x₃(楼层)
神经元1   [ w₁₁       w₁₂        w₁₃    ]   ← 神经元 1 怎么"看"每个特征
神经元2   [ w₂₁       w₂₂        w₂₃    ]   ← 神经元 2 怎么"看"每个特征
 ...        ...
神经元N   [ w_N₁      w_N₂       w_N₃   ]
```
...
`w₁₂ = 0.8`（粗线）= "卧室数"这个特征对 1 号神经元很重要。`w₂₃ = -0.3`（细线）= "楼层"对 2 号神经元是负影响。
...
```
神经元 j 的输出 = ReLU( w_j₁·x₁ + w_j₂·x₂ + w_j₃·x₃ + b_j )
                  = ReLU( Σ(权重 × 特征) + 偏置 )
```
...
| 概念 | v4 | v5 |
|---|---|---|
| 输入 | x（标量，1 维） | x（向量，d 维） |
| 隐藏层权重 | w（N 个数的向量） | W（N×d 矩阵） |
| 神经元前向 | `ReLU(w_i·x + b_i)` | `ReLU(Σ w_ij·x_j + b_i)` |
| 可视化 | 2D 折线 | 3D 曲面 |
| Layer API | `new Layer(1, N, 'relu')` | `new Layer(d, N, 'relu')` |

**矩阵不是新的数学对象——它就是"把多个向量整齐排列在一起"的结果。** 当你把一个特征扩展到多个特征，权重自然就排成了矩阵。代码里你写的是 `w[j][i]`，不是 `w[j*i + something]`。仅此而已。
